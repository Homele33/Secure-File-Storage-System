...
export async function POST(request: Request) {
  ...
  try {
    ...
    console.log(`Uploading encrypted file directly to container: ${containerPath}`);
    ...
    if (containerPath) {
      try {
        console.log(`Attempting to clean up file from container: ${containerPath}`);
        await deleteFileFromContainer(containerPath);
        console.log(`Successfully cleaned up file from container after error`);
      } catch (cleanupError) {
        console.error("Critical: Failed to clean up file from container:", cleanupError);
    ...
    if (error instanceof Error) {
      if (error.message.includes("SFTP") || error.message.includes("container")) {
        return NextResponse.json(
          { message: "Failed to upload to secure storage", error: error.message },
    ...
    return NextResponse.json(
      { message: "File upload failed", error: error instanceof Error ? error.message : "Unknown error" },
      { status: 500 }
    );
  }
}